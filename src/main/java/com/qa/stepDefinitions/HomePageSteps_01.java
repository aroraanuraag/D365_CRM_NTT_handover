...
public class HomePageSteps_01 extends TestBase{
	...
	public static HashMap<String, String> getCredentialsMap() {
		...
		userMap.put("customer", "admin@CRM215677.onmicrosoft.com:pf8POcMEtF"); // Here "customer" is the Key and
																				// "naveenautomation:Test@123" is the
																				// value
		userMap.put("admin", "anuraag:Waheguru@01"); // Here "admin" is the Key and "anuraag:Waheguru@01" is the value
		userMap.put("seller", "suchika:Waheguru@02");
		userMap.put("distributor", "raju:Satnaam@2022");
	...
	@When("^user types \"(.*)\" and clicks Next$")
	public void enter_UserName_and_clicked_Next(String username) throws InterruptedException {
		...
		//driver.findElement(By.xpath("//input[@type='email']")).sendKeys(getUserName(username)); // Code-This will call the  above HashMap.
																							
		driver.findElement(By.xpath("//input[@type='submit']")).click();
	}

		@Then("^user types \"([^\"]*)\" and clicks SignIn button$")
		public void user_types_and_clicks_SignIn_button(String password) throws InterruptedException 
		{
			Thread.sleep(200);
			driver.findElement(By.xpath("//input[@name='passwd']")).sendKeys(password);
			//driver.findElement(By.xpath("//input[@name='passwd']")).sendKeys(getPassword(password)); //Code-This will call the above HashMap
		...
		public void user_clicks_on_the_Yes_option() throws InterruptedException
		{
			Thread.sleep(100);
	...
	@Then("^user verifies the Dynamics CRM home page title Page$")
...
@When("^on the Home Page user clicks on Contacts Menu$")
public void on_the_Home_Page_user_clicks_on_Contacts_Menu() throws InterruptedException  {
	Thread.sleep(3000);
...
@Then("^user enters \"([^\"]*)\" in the search box and clicks Enter$")
public void user_enters_in_the_search_box_and_clicks_Enter(String firstname) throws InterruptedException {
	Thread.sleep(500);
	driver.manage().timeouts().implicitlyWait(10000, TimeUnit.SECONDS);
...
public void user_verifies_Contact_Search_functionality_is_pass_or_fail() throws InterruptedException {
	
	Thread.sleep(1000);
	driver.manage().timeouts().implicitlyWait(10000, TimeUnit.SECONDS);
	//WebDriverWait wait5 = new WebDriverWait(driver, 10000);
    //WebElement verify_ContactSearched = wait5.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@title='STEVE WAGH']")));
	//WebElement confirmed_ContactCreated = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//button[@aria-label='Assign']")));
	//Assert.assertEquals(true, verify_ContactSearched.isDisplayed());
	
	//String verify_ContactSearched = driver.findElement(By.xpath("//a[@title='STEVE WAGH']")).getText();
	//Assert.assertEquals(true, driver.findElement(By.xpath("//a[@title='STEVE WAGH']")).isDisplayed());
	Assert.assertEquals(true, driver.findElement(By.xpath("//a[contains(@title,'Steve Wagh')]")).isDisplayed());
	
	//Assert.assertEquals("STEVE WAGH", verify_ContactSearched);
